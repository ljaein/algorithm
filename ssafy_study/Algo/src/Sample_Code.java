	...
	public static void main(String[] args) {
		...
		try {
			...
			while (socket != null) {
				...
				double disx = balls[ballnum][0] - balls[0][0];
				double disy = balls[ballnum][1] - balls[0][1];
				double len = Math.sqrt(Math.pow(disx, 2) + Math.pow(disy, 2));
				double angle = 0;
				double power = 0;

				angle = 360 + (Math.atan2(disx, disy) * 180) / Math.PI + 1.8;
				if (angle > 360)
					angle -= 360;
				power = len * 0.7;
				if (len < 30)
